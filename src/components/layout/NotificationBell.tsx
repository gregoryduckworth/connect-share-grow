...
const NotificationBell = () => {
  ...
  const { user } = useAuth();
  const [notifications, setNotifications] = useState<Array<Notification & { readAt?: Date }>>([]);
  ...
  const markAllAsRead = async () => {
    ...
    try {
      await Promise.all(
        notifications
          .filter((n) => !n.readAt)
          .map((n) => api.markNotificationAsRead(user.id, n.id)),
      );
  ...
  const handleNotificationClick = async (notification: Notification & { readAt?: Date }) => {
    ...
    } else if (notification.communityId && !notification.postId) {
